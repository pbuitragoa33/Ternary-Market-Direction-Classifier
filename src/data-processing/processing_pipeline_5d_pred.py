# 13. Processing Pipeline script to clean and transform the project data for the 5-day prediction 

# In this file, the idea is to do the following:

#  - Load 1 file (5d_pred_data_enriched.csv) from the S3 bucket (in silver folder)
...
from configs.load_enriched_data_5d_pred import load_data
...
S3_KEY_SILVER = "silver/5d_pred_data_enriched.csv"
S3_PREFIX_GOLD = "gold/"
S3_KEY_ARTIFACTS = "artifacts/processing_artifacts_for_5d_pred.joblib"
...
def execute_pipeline(key_entry = S3_KEY_SILVER,
    ...
    load_processed_data_to_S3(X_train, key = f"{prefix_output}X_train_5d.csv")
    load_processed_data_to_S3(y_train.to_frame(), key = f"{prefix_output}y_train_5d.csv")
    load_processed_data_to_S3(X_validation, key = f"{prefix_output}X_validation_5d.csv")
    load_processed_data_to_S3(y_validation.to_frame(), key = f"{prefix_output}y_validation_5d.csv")
    load_processed_data_to_S3(X_test, key = f"{prefix_output}X_test_5d.csv")
    load_processed_data_to_S3(y_test.to_frame(), key = f"{prefix_output}y_test_5d.csv")
    ...
    local_artifacts_path = os.path.join(ARTIFACTS_LOCAL_PATH, "processing_artifacts_5d_pred.joblib")
    ...
    metadata = {
        ...
        "keys_gold": [f"{prefix_output}{n}_5d.csv" for n in ["X_train", "y_train", "X_validation", "y_validation", "X_test", "y_test"]],
