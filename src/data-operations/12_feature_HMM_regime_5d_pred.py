# 12. Add HMM regime as a feature to the consolidated dataframe (for 5 day predictionb)
...
S3_KEY_SILVER_UNIFIED = "silver/5d_pred_data_unified.csv"
S3_KEY_SILVER_ENRICHED_NEW = "silver/5d_pred_data_enriched.csv"

S3_KEY_ARTIFACTS = "models/5d_pred_hmm_artifacts.joblib"
...
def tuning_apply_PCA(train_df, val_df, test_df,
    ...
    ax.set_title("Model with PCA: Explained Variance Analysis for HMM Regime Detection for 5-Day Prediction", fontsize = 14)
    ax.set_xlim(0, len(n_components))
    ax.set_ylim(0, 102)
    ax.legend()
    ax.grid(True, alpha = 0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(path_to_save_plot, "5d_pred_model_with_PCA_exp_var_analysis.png"))
    plt.show()
...
def fit_HMM_identify_regimes(train_data, val_data, test_data,
                             ...
                             model_name = "model",
                             plot_filename = "5d_pred_HMM_regimes.png"):
    ...
    for ax, (name, ref, reg) in zip(
        ...
        ax.set_title(f"{model_name} | {name}: Close_log_return by HMM Regime for 5-Day Prediction", fontsize = 10)
...
def save_fitted_artifacts(artifacts, local_dir = FT_ENGINEERING_PATH,
    ...
    local_path = os.path.join(local_dir, "5d_pred_hmm_artifacts.joblib")
...
if __name__ == "__main__":
    ...
    df_train_pca_regimes, df_validation_pca_regimes, df_test_pca_regimes, model_GHMM_pca, remap_pca = \
        fit_HMM_identify_regimes(
            ...
            ref_train, ref_val, ref_test,
            model_name = "PCA",
            plot_filename = "5d_pred_HMM_regimes_PCA.png"
        )
    ...
    df_train_all_regimes, df_validation_all_regimes, df_test_all_regimes, model_GHMM_all, remap_all = \
        fit_HMM_identify_regimes(
            ...
            plot_filename = "5d_pred_HMM_regimes_ALL.png"
        )
