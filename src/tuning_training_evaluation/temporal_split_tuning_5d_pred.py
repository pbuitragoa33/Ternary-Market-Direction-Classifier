# Hyperparameters tuning for Temporal Split for 5 day prediction
#
# This file aims to:
...
files_in_gold = ["gold/X_train_5d.csv", "gold/y_train_5d.csv"]

# W&B configuration

WANDB_PROJECT = os.getenv("WANDB_PROJECT_5D")
...
def save_optuna_plots(study, model_name):
    ...
    for plot_fn, suffix in [(plot_optimization_history, "history"),
        ...
        try:
            ...
            fig = ax.figure
            fig.tight_layout()
            path = os.path.join(TUNING_PATH, f"optuna_{model_name}_{suffix}_5d_pred.png")
...
def run_study(model_name, X, y, cv, n_trials = N_TRIALS, timeout = TIMEOUT, metric = PRIMARY_METRIC):
    ...
    json_path = os.path.join(TUNING_PATH, f"best_params_{model_name}_5d_pred.json")

    with open(json_path, "w") as f:

        json.dump(result, f, indent = 2)

    study_path = os.path.join(TUNING_PATH, f"study_{model_name}_5d_pred.joblib")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("5D Pred: Starting hyperparameters tuning with temporal split")
    print("=" * 60)
    ...
    summary_path = os.path.join(TUNING_PATH, "tuning_summary_5d_pred.json")
